...
class ConditionalUNet(nn.Module):
    ...
    def forward(self, x, condition):
        ...
        print(x.shape)
        x = F.relu(nn.Linear(16384, 8192, device=x_concat.device)(x))
        print(x.shape)
        ...
        print(x.shape)
        x = F.relu(nn.Linear(4096, 8192, device=x_concat.device)(x))
        print(x.shape)
        x = F.relu(nn.Linear(8192, 16384, device=x_concat.device)(x))
        print(x.shape)
        x = F.relu(nn.Linear(16384, 32768, device=x_concat.device)(x))
        print(x.shape)
        x = F.relu(nn.Linear(32768, self.fc_in_features, device=x_concat.device)(x))
        print(x.shape)
