...
class ConditionalUNet(nn.Module):
    ...
    def forward(self, x, condition):
        ...
        print("FC")
        x = F.relu(nn.Linear(self.fc_in_features + weight_template_size, 32768, device=x_concat.device)(x_concat))
        print(x.shape)
        x = F.relu(nn.Linear(32768, 16384)(x))
        print(x.shape)
        x = F.relu(nn.Linear(16384, 8192)(x))
        print(x.shape)
        x = F.relu(nn.Linear(8192, 4096)(x))
        print(x.shape)
        x = F.relu(nn.Linear(4096, 2048)(x))
        print(x.shape)
        x = F.relu(nn.Linear(2048, 1024)(x))
        print(x.shape)
        ...
        # x = self.fc3(x)
        # print(x.shape)
        x = F.relu(nn.Linear(512, 1024)(x))
        print(x.shape)
        x = F.relu(nn.Linear(1024, 2048)(x))
        print(x.shape)
        x = F.relu(nn.Linear(2048, 4096)(x))
        print(x.shape)
        x = F.relu(nn.Linear(4096, 8192)(x))
        print(x.shape)
        x = F.relu(nn.Linear(8192, 16384)(x))
        print(x.shape)
        x = F.relu(nn.Linear(16384, 32768)(x))
        print(x.shape)
        x = F.relu(nn.Linear(32768, self.fc_in_features)(x))
        print(x.shape)
