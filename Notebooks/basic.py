...
class DiffusionTrainer:
    ...
    def q_sample(self, x_start, t):
        ...
        # Use the pre-calculated alpha_bar_t on the correct device
...
class DiffusionSampler:
    ...
    def p_sample(self, x, t, condition):
        ...
        alpha_bar_t_prev = self.alpha_bar[t-1] if t > 0 else torch.tensor(1.0, dtype=torch.float32, device=device)
...
weight_template_size = 128  # Assuming your weight template is 128-dimensional
...
images = np.load("Datasets/IITD Palmprint V1/Preprocessed/Left/X_train.npy")  # Shape: (N, 150, 150, 1)
weight_templates = np.load("Datasets/IITD Palmprint V1/Preprocessed/Left/X_train_pca_128.npy")  # Shape: (N, 128)
...
user_weight_template = weight_templates[0]  # Shape: (128,)
user_weight_template = torch.tensor(user_weight_template).float().unsqueeze(0)

# Generate deepfake
generated_image = sampler.sample(user_weight_template, img_shape=input_shape)
...
image = Image.fromarray((generated_image * 127.5 + 127.5).astype(np.uint8), 'L')  # Convert to PIL Image object
